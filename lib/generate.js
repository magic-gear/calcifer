...
module.exports = async function (context, { pkg, options }) {
  const { features, projectType } = options
...
*.sln
*.sw?`.split('\n')
  files['.gitignore'] = configTransforms.lines.write({ value: gitignore })

  if (isLibrary) {
    fs.copySync(path.resolve(__dirname, '../templates/library'), context)
  } else {
    fs.copySync(path.resolve(__dirname, '../templates/application'), context)
  }
